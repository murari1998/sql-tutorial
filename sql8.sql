...
-- que=> i need to get  all the columns for the actor whose actor id > the actor id of a person whose first_name is ed and last_name is chase.

select * from actor;

select * from actor where first_name like 'ed%' and last_name like '%chase';
select * from actor where actor_id > (select actor_id from actor where first_name like 'ed%' and last_name like '%chase');

-- que2=> get all the payment detaile for those payment where the month = the payment month of payment id 5.
...
-- que3=> get the customer_id,payment_id and the amount for the payment where the rental_id< the rental_id of payment id 7 and amount <5;

select * from payment where payment_id= 7 and amount <5; 
select * from payment where rental_id < (select rental_id from payment where payment_id= 7 and amount <5);

-- que4=> get the amount and totoal number of payment done where the total number of payment > the total number of payment done by amount by 5.99.
