...
-- partion by is a keyword useed inside the over()

-- partion by used to devide or separate the similor value like group by          
...
-- order by ==> order by used inside the over()
-- used to spicifie the order of row in each partision


select * from employeesales;

select *,
sum(saleamount) over(order by saleamount) from employeesales; --  thhis is runing sum not used partion by
...
select *, 
sum(amount) over(order by amount) from payment;
...
select *,
avg(amount) over(partition by payment_id order by amount) from payment;    
